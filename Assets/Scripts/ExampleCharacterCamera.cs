    ...
    public class ExampleCharacterCamera : MonoBehaviour
    {
        ...
        public void UpdateWithInput(float deltaTime, float zoomInput, Vector3 rotationInput)
        {
            if (FollowTransform)
            {
                ...
                if (Input.GetMouseButton(1) || start == true)
                {
                ...
                //else if(sdfd == true)
                //{
                //    float mouseX = SimpleInput.GetAxis("Mouse X") * sensitive;
                //    float mouseY = -SimpleInput.GetAxis("Mouse Y") * sensitive;

                //    horizontalAngle += mouseX;
                //    verticalAngle = Mathf.Clamp(verticalAngle + mouseY, -89.0f, 89.0f);

                //    player.transform.localRotation = Quaternion.Euler(0.0f, horizontalAngle, 0.0f);
                ...
                Vector3 targetPosition = _currentFollowPosition - ((quaternion * Vector3.forward) * _currentDistance);
